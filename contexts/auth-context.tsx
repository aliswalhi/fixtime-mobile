...
import { setApiClientAuthToken } from '../services/http/api-client';
...
export default function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    async function bootstrapSession() {
      try {
        ...
        await waitForFirebaseAuthReady().catch((error) => {
  ...
  const setAuthenticatedSession = useCallback(
    async (session: LoginResponse | RegisterResponse) => {
      ...
      queryClient.setQueryData(AUTH_PROFILE_QUERY_KEY, session.profile ?? null);
    },
